    ...
    public partial class FrmTicTacToe : Form
    {
        ...
        private void FillCell(Panel panel, int row, int column)
        {
            ...
            else
            {
                ...
                {
                    Cell cellChoosenByCpu = engine.MakeComputerTurnAndGetCell();
                    if (!cellChoosenByCpu.IsErrorCell())
                    {
                        Panel panelCell = GetPanelCellControlByCell(cellChoosenByCpu);
                        if (panelCell != null)
                        {
                            FillCell(panelCell, cellChoosenByCpu.Row, cellChoosenByCpu.Column);
                        }
                        else
                        {
                            MessageBox.Show("Произошла ошибка: выбранная компьютером клетка недолжна быть равна null!", "Крестики-Нолики", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        }
                    }
                    else
                    {
                        MessageBox.Show("Произошла ошибка: компьютер не смог выбрать клетку для хода!", "Крестики-Нолики", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                }
            }
        }
        ...
        {
            RegularButtonMouseOver(panelPlayerVsCpu, labelPlayeVsCpu);
        }

        private void panelPlayerVsCpu_MouseLeave(object sender, EventArgs e)
        {
            RegularButtonMouseOut(panelPlayerVsCpu, labelPlayeVsCpu);
        }
        ...
        private void labelPlayeVsCpu_Click(object sender, EventArgs e)
        {
            StartNewGameInSelectedMode(GameEngine.GameMode.PlayerVsCpu);
        }

        private void labelPlayeVsCpu_MouseEnter(object sender, EventArgs e)
        {
            RegularButtonMouseOver(panelPlayerVsCpu, labelPlayeVsCpu);
        }
